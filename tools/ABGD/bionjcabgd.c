...
void Initialize (float **delta, struct DistanceMatrix distMat, POINTERS *trees);
...
//  float distance;
//  char name_taxon[LEN];                             /* taxons name      */
  WORD *name;
//printf("initialising\n");  fflush(stdout);
  for(lig=1; lig <= distMat.n; lig++)
    {
//   printf("%d<BR>",lig);fflush(stdout);
      name=(WORD *)calloc(1,sizeof(WORD));            /* taxons name is   */
      if(name == NULL)                                /* put in trees      */
	{
	  printf("Out of memories !!");fflush(stdout);
	  exit(0);
	}
      else
	{
	  	
	  strcpy(name->name,distMat.names[lig-1]);
	  name->suiv=NULL;
	  trees[lig].head=name;
	  trees[lig].tail=name;
	  for(col= 1; col <= distMat.n; col++)
	    {	
//	    printf("%d,",col);
	      delta[lig][col]=distMat.dist[lig-1][col-1];
	    }
	 fflush(stdout);   
	}
    }
...
void Print_output(int i, POINTERS *trees, FILE *output)
{
  ...
  while(parcour != NULL)
    {
      fprintf(output,"%s",parcour->name); // whites are size for printing 'group N°    '
...
//  printf("allocs done<BR>\n");fflush(stdout);

  output= fopen(file_out,"w");
  if (output==NULL)
  	{printf("no file %s<BR>\n",file_out);fflush(stdout);exit(1);}
n=distMat.n; 
...
//while (1)
 //   {
      ...
      *b=0;
  //    printf("init<BR>\n");fflush(stdout);
      Initialize(delta, distMat, trees);
 //     printf("init done<BR>\n");fflush(stdout);
